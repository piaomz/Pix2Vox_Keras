...
randombgs=glob.glob(cfg.DIR.RANDOM_BG_PATH)
...
class RandomBackground(object):
    def __init__(self, random_bg_color_range, random_bg_folder_path=None):
        ...
        if random_bg_folder_path is not None:
            ...
            self.random_bg_files = randombgs
            #print("RandomBG Load "+str(len(self.random_bg_files)))
    ...
    def __call__(self, rendering_images):
        ...
        if len(self.random_bg_files) > 0:
            ...
            #file.write(random_bg_file_path)
            #file.close()
            readbg=cv2.imread(random_bg_file_path)
            if(readbg is None):
                while True:
                    random_bg_file_path = random.choice(self.random_bg_files)
                    readbg=cv2.imread(random_bg_file_path)
                    if(readbg is not None):
                        break
            readbg = cv2.resize(readbg, (cfg.CONST.IMG_W , cfg.CONST.IMG_H))
            random_bg = readbg.astype(np.float32) / 255.
            
            #try:
                #random_bg = cv2.imread(random_bg_file_path).astype(np.float32) / 255.
                #random_bg = cv2.resize(random_bg, (cfg.CONST.IMG_W , cfg.CONST.IMG_H))
            #except:
                #file=open('log/log.txt',mode='w')
                #file.write(random_bg_file_path)
                #file.close()
        ...
        for img_idx, img in enumerate(rendering_images):
            ...
            alpha = 1- (np.expand_dims(img[:, :, 3], axis=2).astype(np.float32))
            img = img[:, :, :3]
